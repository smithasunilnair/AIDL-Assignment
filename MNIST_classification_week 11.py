...
y_pred = y_pred.argmax(axis = 1)  # changing the values in exp to integer
...
#compare test data index wise through visualization imshow
plt.imshow(X_test[0])

model.predict(X_test[0])  #7th value in the array is the highest and hence the  MLP predicted correctly as 2
...
# focus on diagnols -- 974 test image digits are correctly predicted as 0, 1125 test data was correctly predicted as 1 etc  -- differs
